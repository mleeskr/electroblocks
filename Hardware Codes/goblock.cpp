...
SoftwareSerial greenToESP32(11, 10); // RX, TX (Connect Pin 10 to diode stripe)
...
String blockID = "G"; // Identity Definition Flag
...
void setup() {
  ...
  Serial.begin(9600);
  greenToESP32.begin(4800);
  Serial.println("Green Block Online");
}
...
void loop() {
  ...
  if (currentMillis - lastHeartbeat >= dynamicHeartbeatInterval) {
    ...
    Serial.println("Heartbeat Sent: Green Block CONNECTED");
